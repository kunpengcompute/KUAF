...
static hpre_sm2_engine_ctx_t *wd_sm2_new_engine_ctx(KAE_QUEUE_DATA_NODE_S *q_node, hpre_sm2_priv_ctx_t *priv_ctx)
{
    ...
    if (e_sm2_ctx == NULL) {
        KUAF_ERROR("OPENSSL_malloc ctx failed");
        return NULL;
    }
    memset(e_sm2_ctx, 0, sizeof(hpre_sm2_engine_ctx_t));
...
void wd_sm2_put_engine_ctx(hpre_sm2_engine_ctx_t *e_hpre_sm2_ctx)
{
    if (unlikely(e_hpre_sm2_ctx == NULL)) {
        KUAF_WARN("sec cipher engine ctx NULL!");
        return;
    }
...
hpre_sm2_engine_ctx_t *wd_sm2_get_engine_ctx(hpre_sm2_priv_ctx_t *priv_ctx)
{
    ...
    if (unlikely(priv_ctx == NULL)) {
        KUAF_ERROR("sm2 cipher priv ctx NULL!");
        return NULL;
    }
    KUAF_DEBUG("kae hpre_sm2 get queue node from pool start.");

    q_node = kae_get_node_from_pool(g_hpre_sm2_qnode_pool);
    if (q_node == NULL) {
        KUAF_ERROR_LIMIT("failed to get hardware queue");
        return NULL;
    }
    ...
    if (e_hpre_sm2_ctx == NULL) {
        ...
        if (e_hpre_sm2_ctx == NULL) {
            KUAF_WARN("sec new engine ctx fail!");
    ...
    // 初始化engine参数
    if (wd_sm2_init_engine_ctx(e_hpre_sm2_ctx) == -1) {
        KUAF_WARN("init engine ctx fail!");
...
{
    const unsigned char sm2_order[] = {0xff, 0xff, 0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    ...
    if (smctx->e_hpre_sm2_ctx == NULL) {
        ...
        if (smctx->e_hpre_sm2_ctx == NULL) {
            KUAF_WARN("hpre sm2 failed to get engine ctx, switch to soft cipher");
            return OPENSSL_FAIL;
        }
    }
...
static int hpre_sm2_ctrl_str(EVP_PKEY_CTX *ctx, const char *type, const char *value)
{
    if (strcmp(type, "ec_paramgen_curve") == 0) {
        int nid;

        if ((EC_curve_nist2nid(value) == NID_undef) && (OBJ_sn2nid(value) == NID_undef) &&
...
static int hpre_sm2_check_digest_evp_lib(const EVP_MD *digest, EVP_MD_CTX *hash, const size_t id_len, const uint8_t *id)
{
    uint8_t e_byte;
    uint16_t entl;
...
static int check_equation_param(struct hpre_sm2_param *param, EVP_MD_CTX *hash, uint8_t *buf, int p_bytes)
{
    if (BN_bn2binpad(param->a, buf, p_bytes) < 0 || !EVP_DigestUpdate(hash, buf, p_bytes) ||
        BN_bn2binpad(param->b, buf, p_bytes) < 0 || !EVP_DigestUpdate(hash, buf, p_bytes)) {
...
static int check_base_point_group_param(struct hpre_sm2_param *param, BN_CTX *ctx, const EC_KEY *key)
{
    const EC_GROUP *group = EC_KEY_get0_group(key);

    if (!EC_POINT_get_affine_coordinates(group, EC_GROUP_get0_generator(group), param->xG, param->yG, ctx)) {
...
static int check_base_point_param(struct hpre_sm2_param *param, EVP_MD_CTX *hash, uint8_t *buf, int p_bytes)
{
    if (BN_bn2binpad(param->xG, buf, p_bytes) < 0 || !EVP_DigestUpdate(hash, buf, p_bytes) ||
        BN_bn2binpad(param->yG, buf, p_bytes) < 0 || !EVP_DigestUpdate(hash, buf, p_bytes)) {
...
static int check_pkey_point_group_param(struct hpre_sm2_param *param, BN_CTX *ctx, const EC_KEY *key)
{
    const EC_GROUP *group = EC_KEY_get0_group(key);

    if (!EC_POINT_get_affine_coordinates(group, EC_KEY_get0_public_key(key), param->xA, param->yA, ctx)) {
...
static int check_pkey_point_param(struct hpre_sm2_param *param, EVP_MD_CTX *hash, uint8_t *buf, int p_bytes,
                                  uint8_t *out)
{
    if (BN_bn2binpad(param->xA, buf, p_bytes) < 0 || !EVP_DigestUpdate(hash, buf, p_bytes) ||
        BN_bn2binpad(param->yA, buf, p_bytes) < 0 || !EVP_DigestUpdate(hash, buf, p_bytes) ||
...
static int hpre_sm2_compute_z_digest(uint8_t *out, const EVP_MD *digest, const uint8_t *id, const size_t id_len,
                                     const EC_KEY *key)
{
    const EC_GROUP *group = EC_KEY_get0_group(key);
    ...
    if (!check_equation_param(param, hash, buf, p_bytes) || !check_base_point_group_param(param, ctx, key) ||
...
static int hpre_sm2_digest_custom(EVP_PKEY_CTX *ctx, EVP_MD_CTX *mctx)
{
    ...
    if (!smctx->ctx.id_set)
        (void)hpre_sm2_ctrl(ctx, EVP_PKEY_CTRL_SET1_ID, SM2_DEFAULT_USERID_LEN, (void *)SM2_DEFAULT_USERID);
...
static int openssl_soft_encrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                                size_t inlen)
{
...
static int openssl_soft_decrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                                size_t inlen)
{
    const EVP_PKEY_METHOD *openssl_meth;
...
static int hpre_sm2_ciphertext_size(const EC_KEY *key, const EVP_MD *digest, size_t msg_len, size_t *ct_size)
{
    ...
    sz = ECC_POINT_SIZE(ASN1_object_size(0, field_size + 1, V_ASN1_INTEGER)) +
         ASN1_object_size(0, md_size, V_ASN1_OCTET_STRING) + ASN1_object_size(0, msg_len, V_ASN1_OCTET_STRING);
...
static int hpre_sm2_encrypt_check(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                                  size_t inlen)
{
    KUAF_DEBUG("sm2_encrypt_check started.\n");
    struct hpre_sm2_priv_ctx *smctx = EVP_PKEY_CTX_get_data(ctx);
    EVP_PKEY *p_key = EVP_PKEY_CTX_get0_pkey(ctx);
    EC_KEY *ec = EVP_PKEY_get0(p_key);
    const EVP_MD *md;
...
static int hpre_sm2_encrypt_init_iot(void *sess, struct wcrypto_ecc_op_data *opdata, unsigned char *in, size_t inlen)
{
...
int hpre_sm2_set_public_key(void *sess, const EC_KEY *eckey)
{
    ...
    len = EC_POINT_point2buf(group, point, SM2_OCTET_STRING, &point_bin, NULL);
    if (!len) {
...
int hpre_sm2_crypto(struct wcrypto_ecc_op_data *opdata, struct hpre_sm2_priv_ctx *smctx)
{
    ...
    KUAF_DEBUG("hpre rsa no async Job or async disable, back to sync!");
    async_cleanup_op_done_v1(&op);
    return hpre_sync_do_sm2(smctx->e_hpre_sm2_ctx, opdata);
}

static int sm2_cipher_bin_to_ber(const EVP_MD *md, struct wcrypto_ecc_point *c1, struct wd_dtb *c2, struct wd_dtb *c3,
                                 unsigned char *ber, size_t *ber_len)
{
    ...
    ciphertext_leni = i2d_HPRE_SM2_Ciphertext(&ctext_struct, (unsigned char **)&ber);
...
static int hpre_sm2_encrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                            size_t inlen)
{
    struct hpre_sm2_priv_ctx *smctx = EVP_PKEY_CTX_get_data(ctx);
    ...
    if (!ret) {
        KUAF_ERROR("hpre_sm2_encrypt_check failed ,then switch to soft!\n");
        goto do_soft;
    }
    ...
    if (!ret) {
        KUAF_ERROR("sm2_encrypt_init_iot failed , then switch to soft!\n");
        goto do_soft;
    }
    ...
    if (!ret) {
        ret = OPENSSL_FAIL;
        KUAF_ERROR("update_public_key failed , then switch to soft!\n");
        goto uninit_iot;
    }
    ...
    if (!ret) {
        ...
        KUAF_ERROR("uadk_ecc_crypto failed.\n");
        goto uninit_iot;
    }
...
static int hpre_sm2_decrypt_init_iot(void *sess, struct wcrypto_ecc_op_data *opdata, struct wcrypto_ecc_point *c1,
...
static int hpre_sm2_get_plaintext(struct wcrypto_ecc_op_data *opdata, unsigned char *out, size_t *outlen)
{
...
static int hpre_sm2_decrypt_check(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                                  size_t inlen)
{
    struct hpre_sm2_priv_ctx *smctx = EVP_PKEY_CTX_get_data(ctx);
    const EVP_MD *md;
...
static int hpre_sm2_decrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                            size_t inlen)
{
    struct hpre_sm2_priv_ctx *smctx = EVP_PKEY_CTX_get_data(ctx);
...
static int hpre_sm2_sign_init_iot(void *sess, struct wcrypto_ecc_op_data *opdata, unsigned char *digest,
                                  size_t digest_len)
{
    struct wcrypto_ecc_out *ecc_out;
    struct wcrypto_ecc_in *ecc_in;
...
static int hpre_sm2_sign_check(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs,
                               size_t tbslen)
{
    struct hpre_sm2_priv_ctx *smctx = EVP_PKEY_CTX_get_data(ctx);
    EVP_PKEY *p_key = EVP_PKEY_CTX_get0_pkey(ctx);
...
static int hpre_sm2_sign_bin_to_ber(EC_KEY *ec, struct wd_dtb *r, struct wd_dtb *s, unsigned char *sig, size_t *siglen)
{
    int ret = OPENSSL_FAIL;
    ECDSA_SIG *e_sig;
    BIGNUM *br, *bs;
...
static int openssl_soft_sign(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs,
                             size_t tbslen)
{
    const EVP_PKEY_METHOD *openssl_meth;
...
static int hpre_sm2_sign(EVP_MD_CTX *mctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs, size_t tbslen)
{
    ...
    ret = get_dev_by_sync_mode(siglen);
    memset(&opdata, 0, sizeof(opdata));
...
uninit_iot:
    ...
    wcrypto_del_ecc_out(smctx->e_hpre_sm2_ctx->wd_ctx, opdata.out);
    if (ret == OPENSSL_SUCCESS) {
        KUAF_DEBUG("sm2_sign successed!\n");
        return OPENSSL_SUCCESS;
    }
do_soft:
    fprintf(stderr, "sm2_sign failed, switch to execute openssl software calculation.\n");
    KUAF_ERROR("sm2_sign failed, switch to execute openssl software calculation.\n");
...
static int hpre_sm2_verify_check(EVP_PKEY_CTX *ctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                                 size_t tbslen)
{
    struct hpre_sm2_priv_ctx *smctx = EVP_PKEY_CTX_get_data(ctx);
...
static int hpre_sm2_verify_init_iot(void *sess, struct wcrypto_ecc_op_data *opdata, struct wd_dtb *e, struct wd_dtb *r,
                                    struct wd_dtb *s)
{
    struct wcrypto_ecc_in *ecc_in;
...
static int hpre_sm2_sig_ber_to_bin(EC_KEY *ec, unsigned char *sig, size_t sig_len, struct wd_dtb *r, struct wd_dtb *s)
{
...
static int openssl_soft_verify(EVP_PKEY_CTX *ctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                               size_t tbslen)
{
    const EVP_PKEY_METHOD *openssl_meth;
...
static int hpre_sm2_verify(EVP_MD_CTX *mctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                           size_t tbslen)
{
    EVP_PKEY_CTX *ctx = EVP_MD_CTX_pkey_ctx(mctx);
    struct hpre_sm2_priv_ctx *smctx = EVP_PKEY_CTX_get_data(ctx);
    ...
    if (!ret) {
        KUAF_ERROR("sm2_verify_check failed.\n");
        goto do_soft;
    }
    ...
    if (!ret) {
        ret = OPENSSL_FAIL;
        KUAF_ERROR("sm2_verify_check failed,switch to soft.\n");
        goto uninit_iot;
    }
    ...
    if (!ret) {
        ...
        KUAF_ERROR("uadk_ecc_crypto failed,switch to soft.\n");
        goto uninit_iot;
    }
    ret = OPENSSL_SUCCESS;
...
uninit_iot:
    wcrypto_del_ecc_in(smctx->e_hpre_sm2_ctx->wd_ctx, opdata.in);
    if (ret == OPENSSL_SUCCESS) {
        KUAF_DEBUG("sm2_verify successed!\n");
        return OPENSSL_SUCCESS;
    }
do_soft:
    fprintf(stderr, "sm2_verify failed,switch to execute openssl software calculation.\n");
    KUAF_ERROR("sm2_verify failed,switch to execute openssl software calculation.\n");
...
static EVP_PKEY_METHOD *hpre_sm2_create_pmeth()
{
    ...
    EVP_PKEY_meth_set_digestsign(meth, hpre_sm2_sign);
   EVP_PKEY_meth_set_digestverify(meth, hpre_sm2_verify);
    return meth;
}
...
int wd_sm2_init_qnode_pool(void)
{
    ...
    if (g_hpre_sm2_qnode_pool == NULL) {
        KUAF_ERROR("hpre rsa qnode poll init fail!\n");
        return KUAF_ERROR;
    }
