...
static int kuaf_rsa_public_encrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding);

static int kuaf_rsa_private_encrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding);

static int kuaf_rsa_public_decrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding);

static int kuaf_rsa_private_decrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding);

static int kuaf_rsa_keygen(RSA *rsa, int bits, BIGNUM *e, BN_GENCB *cb);

static int kuaf_rsa_mod_exp(BIGNUM *r0, const BIGNUM *I, RSA *rsa, BN_CTX *ctx);

static int kuaf_bn_mod_exp(
...
RSA_METHOD *kuaf_get_rsa_methods(void)
{
    ...
    if (g_hpre_rsa_method == NULL) {
        ...
        KUAF_ERROR("Failed to allocate HPRE RSA methods");
        return NULL;
    }

    ret &= RSA_meth_set_pub_enc(g_hpre_rsa_method, kuaf_rsa_public_encrypt);
    ret &= RSA_meth_set_pub_dec(g_hpre_rsa_method, kuaf_rsa_public_decrypt);
    ret &= RSA_meth_set_priv_enc(g_hpre_rsa_method, kuaf_rsa_private_encrypt);
    ret &= RSA_meth_set_priv_dec(g_hpre_rsa_method, kuaf_rsa_private_decrypt);
    ret &= RSA_meth_set_keygen(g_hpre_rsa_method, kuaf_rsa_keygen);
    ret &= RSA_meth_set_mod_exp(g_hpre_rsa_method, kuaf_rsa_mod_exp);
    ret &= RSA_meth_set_bn_mod_exp(g_hpre_rsa_method, kuaf_bn_mod_exp);

    if (ret == 0) {
        KAEerr(KAE_F_HPRE_GET_RSA_METHODS, KAE_R_RSA_SET_METHODS_FAILURE);
        KUAF_ERROR("Failed to set HPRE RSA methods");
        return NULL;
    }
...
int kuaf_module_init(void)
{
    ...
    (void)kuaf_get_rsa_pkey_meth();
    (void)kuaf_get_rsa_methods();
...
EVP_PKEY_METHOD *kuaf_get_rsa_pkey_meth(void)
{
    ...
    if (g_hpre_pkey_meth == NULL) {
        ...
        if (g_hpre_pkey_meth == NULL) {
            KUAF_ERROR("failed to new pkey meth");
            return NULL;
        }
...
static int hpre_check_meth_args(EVP_PKEY_METHOD **pmeth, const int **pnids, int nid)
{
...
static int hpre_rsa_check(const int flen, const BIGNUM *n, const BIGNUM *e, int *num_bytes, RSA *rsa)
{
    int key_bits;
    ...
    *num_bytes = BN_num_bytes(n);
    if (flen > *num_bytes) {
        ...
        KUAF_WARN("data length is large than num bytes of rsa->n");
        return HPRE_CRYPTO_FAIL;
    }

    key_bits = RSA_bits(rsa);
    if (!check_bit_useful(key_bits)) {
        KUAF_WARN("op sizes not supported by hpre engine then back to soft!");
        return HPRE_CRYPTO_FAIL;
    }
...
static int hpre_rsa_prepare_opdata(
    const BIGNUM *n, int flen, const unsigned char *from, BN_CTX **bn_ctx, BIGNUM **bn_ret, BIGNUM **f_ret)
{
    BN_CTX *bn_ctx_tmp;
...
static int kuaf_rsa_public_encrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding)
{
    ...
    if (eng_ctx == NULL) {
        KUAF_WARN("get eng ctx fail then switch to soft!");
        rsa_soft_mark = 1;
        goto end_soft;
    }
    ret = get_dev_by_sync_mode(flen);
    RSA_get0_key(rsa, &n, &e, &d);
    ret = check_pubkey_param(n, e);
    GOTOEND_IF(ret != HPRE_CRYPTO_SUCC, "check public key fail", KAE_F_HPRE_RSA_PUBENC, KAE_R_PUBLIC_KEY_INVALID);

    bn_ctx = BN_CTX_new();

    GOTOEND_IF(bn_ctx == NULL, "bn_ctx MALLOC FAILED!", KAE_F_HPRE_RSA_PUBENC, KAE_R_MALLOC_FAILURE);
    BN_CTX_start(bn_ctx);
    ret_bn = BN_CTX_get(bn_ctx);
    num_bytes = BN_num_bytes(n);
    in_buf = (unsigned char *)OPENSSL_malloc(num_bytes);

    GOTOEND_IF(ret_bn == NULL || in_buf == NULL,
        "PUBLIC_ENCRYPT RSA MALLOC FAILED!",
        KAE_F_HPRE_RSA_PUBENC,
        KAE_R_MALLOC_FAILURE);

    ret = hpre_rsa_padding(flen, from, in_buf, num_bytes, padding, PUB_ENC);
    GOTOEND_IF(ret == HPRE_CRYPTO_FAIL, "RSA PADDING FAILED", KAE_F_HPRE_RSA_PUBENC, KAE_R_RSA_PADDING_FAILURE);
    ...
    eng_ctx->opdata.in =
        eng_ctx->rsa_setup.br.alloc(eng_ctx->qlist->kae_queue_mem_pool, eng_ctx->qlist->kae_queue_mem_pool->block_size);
    eng_ctx->opdata.out =
        eng_ctx->rsa_setup.br.alloc(eng_ctx->qlist->kae_queue_mem_pool, eng_ctx->qlist->kae_queue_mem_pool->block_size);
    ...
    GOTOEND_IF(
        ret == HPRE_CRYPTO_FAIL, "hpre rsa pub encrypt failed!", KAE_F_HPRE_RSA_PUBENC, KAE_R_PUBLIC_ENCRYPTO_FAILURE);

    BN_bin2bn((const unsigned char *)eng_ctx->opdata.out, eng_ctx->opdata.out_bytes, ret_bn);
    ret = BN_bn2binpad(ret_bn, to, num_bytes);

    KUAF_DEBUG("hpre rsa public encrypt success!");
...
static int kuaf_rsa_private_encrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding)
{
    ...
    GOTOEND_IF(bn_ctx == NULL, "PRI_ENC MALLOC_FAILURE ", KAE_F_HPRE_RSA_PRIENC, KAE_R_MALLOC_FAILURE);
    ...
    in_buf = (unsigned char *)OPENSSL_malloc(num_bytes);
    GOTOEND_IF(bn_ret == NULL || in_buf == NULL, "OpenSSL malloc failure", KAE_F_HPRE_RSA_PRIENC, KAE_R_MALLOC_FAILURE);

    ret = hpre_rsa_padding(flen, from, in_buf, num_bytes, padding, PRI_ENC);
    GOTOEND_IF(ret == HPRE_CRYPTO_FAIL, "RSA PADDING FAILED!", KAE_F_HPRE_RSA_PRIENC, KAE_R_RSA_PADDING_FAILURE);

    GOTOEND_IF(BN_bin2bn(in_buf, num_bytes, f) == NULL, "BN_bin2bn failure", KAE_F_HPRE_RSA_PRIENC, KAE_R_ERR_LIB_BN);

    ret = BN_ucmp(f, n);
    GOTOEND_IF(ret >= 0, "RSA PADDING FAILED!", KAE_F_HPRE_RSA_PRIENC, KAE_R_DATA_TOO_LARGE_FOR_MODULUS);
    ...
    if (ret == HPRE_CRYPTO_FAIL) {
        KUAF_WARN("hpre rsa priv encrypt failed!");
        rsa_soft_mark = 1;
        goto end;
    }
    ...
    KUAF_DEBUG("hpre rsa priv encrypt success!");
...
BN_ULONG *kuaf_bn_get_words(const BIGNUM *a)
{
	return a->d;
}
static int kuaf_rsa_public_decrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding)
{
    ...
    ret = hpre_rsa_crypto(eng_ctx, &eng_ctx->opdata);
    GOTOEND_IF(
        ret == HPRE_CRYPTO_FAIL, "hpre rsa pub decrypt failed!", KAE_F_HPRE_RSA_PUBDEC, KAE_R_PUBLIC_DECRYPTO_FAILURE);

    BN_bin2bn((const unsigned char *)eng_ctx->opdata.out, eng_ctx->opdata.out_bytes, bn_ret);
    if ((padding == RSA_X931_PADDING) && ((kuaf_bn_get_words(bn_ret)[0] & 0xf) != 12)) {  // not 12 then BN_sub
        GOTOEND_IF(!BN_sub(bn_ret, n, bn_ret), "BN_sub failed", KAE_F_HPRE_RSA_PUBDEC, KAE_R_ERR_LIB_BN);
    }
    len = BN_bn2binpad(bn_ret, buf, num_bytes);
    ret = check_rsa_padding(to, num_bytes, buf, len, padding, PUB_DEC);
    if (ret == HPRE_CRYPTO_FAIL) {
        KUAF_WARN("hpre rsa check padding failed.switch to soft");
        rsa_soft_mark = 1;
        goto end;
    }

    KUAF_DEBUG("hpre rsa public decrypt success!");
...
static int kuaf_rsa_private_decrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding)
{
    ...
    if (flen > num_bytes) {
        ...
        KUAF_ERROR("PRIVATE_DECRYPT DATA_GREATER_THAN_MOD_LEN");
        return HPRE_CRYPTO_FAIL;
    }
    ...
    ret = get_dev_by_sync_mode(flen);
    bn_ctx = BN_CTX_new();
    GOTOEND_IF(bn_ctx == NULL, "bn_ctx MALLOC FAILED!", KAE_F_HPRE_RSA_PRIDEC, KAE_R_ERR_LIB_BN);
...
#endif
    buf = (unsigned char *)OPENSSL_malloc(num_bytes);
    GOTOEND_IF(bn_ret == NULL || buf == NULL,
        "PRIVATE_DECRYPT ERR_R_MALLOC_FAILURE",
        KAE_F_HPRE_RSA_PRIDEC,
        KAE_R_MALLOC_FAILURE);

    GOTOEND_IF(BN_bin2bn(from, (int)flen, f) == NULL, "BN_bin2bn failure", KAE_F_HPRE_RSA_PRIDEC, KAE_R_ERR_LIB_BN);

    GOTOEND_IF(BN_ucmp(f, n) >= 0,
        "PRIVATE_DECRYPT, RSA_R_DATA_TOO_LARGE_FOR_MODULUS",
        KAE_F_HPRE_RSA_PRIDEC,
    ...
    ret = hpre_rsa_crypto(eng_ctx, &eng_ctx->opdata);
    if (ret == HPRE_CRYPTO_FAIL) {
        KUAF_WARN("hpre rsa priv decrypt failed.switch to soft");
        rsa_soft_mark = 1;
        goto end;
    }
    ...
    KUAF_DEBUG("hpre rsa priv decrypt success!");
...
static int kuaf_rsa_keygen(RSA *rsa, int bits, BIGNUM *e, BN_GENCB *cb)
{
    ...
    if (bits < RSA_MIN_MODULUS_BITS) {
        ...
        KUAF_ERROR("RSA_BUILTIN_KEYGEN RSA_R_KEY_SIZE_TOO_SMALL");
        return HPRE_CRYPTO_FAIL;
    }
    ...
    GOTOEND_IF(e_value == NULL || p == NULL || q == NULL,
        "e_value or p or q MALLOC FAILED.",
        KAE_F_HPRE_RSA_KEYGEN,
        KAE_R_ERR_LIB_BN);
    GOTOEND_IF(hpre_rsa_primegen(bits, e, p, q, cb) == OPENSSL_FAIL,
        "hisi_rsa_primegen failed",
        KAE_F_HPRE_RSA_KEYGEN,
        KAE_R_GET_PRIMEKEY_FAILURE);
    GOTOEND_IF(BN_copy(e_value, e) == NULL, "copy e failed", KAE_F_HPRE_RSA_KEYGEN, KAE_R_ERR_LIB_BN);
    ...
    if (ret != 0) {
        KUAF_WARN("hpre_fill_keygen_opdata failed");
        rsa_soft_mark = 1;
        goto end;
    }
    ret = hpre_rsa_sync(eng_ctx->ctx, &eng_ctx->opdata);
    if (ret == HPRE_CRYPTO_FAIL) {
        KUAF_WARN("hpre generate rsa key failed.switch to soft");
        rsa_soft_mark = 1;
        goto end;
    }
    ret = hpre_rsa_get_keygen_param(&eng_ctx->opdata, eng_ctx->ctx, rsa, e_value, p, q);

    KUAF_DEBUG("hpre rsa keygen success!");
